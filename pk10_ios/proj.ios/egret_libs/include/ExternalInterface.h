...
private:
    ...
    static std::map<std::string, int> _externalInterfaceTimesMap;
    
public:
    static void run();
    static void stop();
    static void call(const std::string& name, const std::string& value);
    static void call(NSString* name, NSString* value);
    ...
    static void setExternalInterfaceNames();
    static void onExternalInterfaceCalled(const std::string& funcName);
    
private:
    static void sendFuncInfoToNative();
};
