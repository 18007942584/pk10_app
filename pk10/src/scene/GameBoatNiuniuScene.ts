...
class GameBoatNiuniuScene extends BaseScene {

    private gameData:GameBoatNiuniuData = new GameBoatNiuniuData();
   ...
   //public concoludeImage: eui.Image;
    ...
    private roomType    = RoomType.RoomTypeNiuniu;
    ...
    public constructor() {
        super();
        this.skinName = "resource/skins/scene/GameBoatNiuniuSceneSkin.exml";
    ...
    public createGameScene() {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            image.source = "boat_sea_bg_png";//"Map_png";
            image.width = 920;
            image.height = 340;
            image.x = i * 920;
            image.y = 0;

            let bottomImage=new eui.Image();
            bottomImage.source = "boat_chedao_new_1_png";//"Map_png";
            bottomImage.width = 920;
            bottomImage.height = 40;
            bottomImage.x = i * 920;
            bottomImage.y = 300;

            this.raceBgScrollerGroup.addChild(image);
            this.raceBgScrollerGroup.addChild(bottomImage);
        }
    ...
    private swipeHandler(evt: egret.TouchEvent): void {
        ...
        if (this.movePositionX - this.beginPositionX < -20) {
            ...
            if (this.mainScroller.viewport.scrollH < 1|| this.mainScroller.viewport.scrollH >= 1000) {
                this.showRight();
    ...
    private onNotifySettlementResult(result: PacketSettlementResult) {
        ...
        /*结算
        //显示庄家
        if(this.settlementResult.resultInfoNiuniuHaveBanker!=null) {
            this.setBanker(this.settlementResult.resultInfoNineHaveBanker.bank_area);
        } else {
            this.hideAllBanker();
        }
        */
    ...
    //最多显示筹码数，0为此功能不开启
    private areaChipShowCount:number = CommonConfig.gameConfig.area_chip_show_count*2;
    ...
    private startRaceStepOneStart()//第二步，开始跑车
    {
        if (this.sound_on) {
            this.is_kaijiang = 1;
            this.sound_channel=Director.getInstance().soundPlay("boat_mp3");
        }
    ...
    private startRaceStepThreeEnd() {
        ...
        twStopSound.to({}, 100).call(function(){
            ...
            if(that.sound_on)
            {
                that.sound_channel=Director.getInstance().soundPlay("boat_mp3");
            }
    ...
    //判断牛几
    private getNiuCount(codeList:number[])
    {
        codeList.sort((a, b) => {
            if (a < b) {
                return -1;
        ...
        for (let i = 0; i < COUNT - 1; ++i) {
            for (let j = i + 1; j < COUNT; ++j) {
                if ((codeList[i] + codeList[j]) % 10 == mod) {
                    // 判断牛牛
                    if (mod == 0) mod = 10;
                    return mod;
                }
    ...
    //获取牛字符
    private getNiuText(niuCode:number)
    {
        switch(niuCode)
        {
            case 0:
    ...
    private concludeTimerHandler() {
        ...
        for(let i=0;i<2;i++) {
            let codeList:number[] = new Array();
            codeList[0] = Number(openCodeList[i*5+0]);
            codeList[1] = Number(openCodeList[i*5+1]);
            codeList[2] = Number(openCodeList[i*5+2]);
            codeList[3] = Number(openCodeList[i*5+3]);
            codeList[4] = Number(openCodeList[i*5+4]);
            let checkedValue = this.getNiuCount(codeList);
            let area = i+1;
            ...
            this.showMajiangAnimation(codeList[0], 0, area, delay);
            this.showMajiangAnimation(codeList[1], 1, area, delay);
            this.showMajiangAnimation(codeList[2], 2, area, delay);
            this.showMajiangAnimation(codeList[3], 3, area, delay);
            this.showMajiangAnimation(codeList[4], 4, area, delay);
            
            delay+=2500;
            let tw = egret.Tween.get(this);
            tw.to({}, delay).call(function(){
                let pointAreaLabel = that.getAreaPointLabel(area);
                pointAreaLabel.text = that.getNiuText(checkedValue);
            })
        }
    ...
    private showChipAnimation() {
        ...
        let that = this;
        if(this.settlementResult.resultInfoNiuniuNoBanker!=null) {//无庄
            let delay = 0;
            let areaRemainList:number[] = [];
            areaRemainList[1] = this.gameData.area_summary[1];
            areaRemainList[2] = this.gameData.area_summary[1];
            
            if(this.settlementResult.resultInfoNiuniuNoBanker.area!=null)
            {
                let winArea = this.settlementResult.resultInfoNiuniuNoBanker.area.win;
                let loseArea = this.settlementResult.resultInfoNiuniuNoBanker.area.lose;
                ...
                if(loseAreaRemain>0)
                {
                    ...
                    //移动一部分的币
                    this.showCoinAnimation(Number(loseArea.area), Number(winArea.area), Number(needMoveValue), loseAreaRemain, winAreaRemain, delay);
                    areaRemainList[loseArea.area] = areaRemainList[loseArea.area] - needMoveValue;
                }else
                {//移动全部
                    this.showCoinAnimation(Number(loseArea.area), Number(winArea.area), Number(loseArea.result), loseAreaRemain, winAreaRemain, delay);
        ...
        }else if(this.settlementResult.resultInfoNiuniuHaveBanker!=null) {//有庄
            //结算有问题还要改
            let delay = 0;
            let bankArea: number = this.settlementResult.resultInfoNiuniuHaveBanker.bank_area;

            for (let area_row in this.settlementResult.resultInfoNiuniuHaveBanker.area) {
                let area: ResultClientArea = this.settlementResult.resultInfoNiuniuHaveBanker.area[area_row];
                let winAreaRemain = this.settlementResult.resultInfoNiuniuHaveBanker.banker;
    ...
    private moveChipToPos(chipImage: eui.Image, targetX: number, targetY: number, delay: number, needHideAtLast: boolean) {
        let tw = egret.Tween.get(chipImage);
        let twRun = tw.to({}, delay).to({ x: targetX, y: targetY }, 500);
        if (needHideAtLast) {
    ...
    private showMajiangAnimation(point: number, pos: number, area: number, delay: number) {
        let smallWidth = 50;
        let smallHeight = 75;
        let xPos =  (this.groupCenter.width/5)*(pos)+50;
        ...
        majiangImage.y = -100;
        majiangImage.source = "cardBg_png";//majiang_7_png
        this.groupCenter.addChild(majiangImage);

        let that = this;
        let tw1TargetX = betGroup.x+(50)+50*pos;
        let tw1TargetY = area==1?betGroup.y+20:betGroup.y+betGroup.height+50;
        let tw1 = egret.Tween.get( majiangImage );
        tw1.to({}, delay).to( {x:xPos}, 1000 ).to({source:tempMajiangSource, skewY:0}, 500).to({}, 300).
    ...
    private onRewardResultHandler(evt: egret.TouchEvent): void {
        ...
        Director.getInstance().showRuleFeitingNiuNiudialog(this,"rewardResult","niuniu",this.sound_on,this.gameId,this.roomInfo.is_official);
    }
    ...
    private onRuleHandler(evt: egret.TouchEvent): void {
        ...
        Director.getInstance().showRuleFeitingNiuNiudialog(this, "gameRule", "niuniu",this.sound_on,this.gameId,this.roomInfo.is_official);
    }
    ...
    private concludeHandler(): void {
        ...
        if(this.settlementResult.resultInfoNiuniuNoBanker!=null) {
            resultUserList = this.settlementResult.resultInfoNiuniuNoBanker.users;
            bankerReward = 0;
        }else if(this.settlementResult.resultInfoNiuniuHaveBanker!=null) {
            resultUserList = this.settlementResult.resultInfoNiuniuHaveBanker.users;
            bankerReward = this.settlementResult.resultInfoNiuniuHaveBanker.banker;
        }
    ...
    private hideAllMajiang() {
        ...
        this.majiang_image_0.skewY = 180;
        this.majiang_image_1.skewY = 180;
        this.majiang_image_2.skewY = 180;
        this.majiang_image_3.skewY = 180;
        this.majiang_image_4.skewY = 180;
        this.majiang_image_5.skewY = 180;
        this.majiang_image_6.skewY = 180;
        this.majiang_image_7.skewY = 180;
        this.majiang_image_8.skewY = 180;
        this.majiang_image_9.skewY = 180;
    ...
    private testHandler(evt: egret.TouchEvent): void {
        ...
        */
        ...
        tw.to({}, 0).call(function () {
            ...
            .call(function () {
                //玩家下注
            let msg_bet_3 = "{\"type\":\"bet\",\"msg\":\"\",\"area\":\"1\",\"number\":\"10\",\"area_summary\":[{\"area\":1,\"number\":\"20\"}]}";
            let msg_bet_4 = "{\"type\":\"bet\",\"msg\":\"\",\"area\":\"2\",\"number\":\"10\",\"area_summary\":[{\"area\":2,\"number\":\"20\"}]}";
            ...
            .call(function () {
            //再推送消息: 开奖结算:牛牛无庄家
            let msg_conclude = "{\"type\":\"settlement_result\",\"room_type\":\"2\",\"info\":{\"mode\":0,\"banker\":0,\"area\":{\"win\":{\"id\":579,\"area\":1,\"bet\":\"200\",\"result\":\"200\"},\"lose\":{\"id\":579,\"area\":2,\"bet\":\"150\",\"result\":\"150\"}},\"users\":[{\"id\":1,\"nickname\":\"玩家17111920193170\",\"number\":300},{\"id\":2,\"nickname\":\"玩家17112423235566\",\"number\":-100},{\"id\":3,\"nickname\":\"玩家17112521543668\",\"number\":-200}]},\"open_result\":{\"expect\":\"653194\",\"opencode\":\"08,03,01,04,09,10,05,07,02,06\",\"opentime\":\"2017-11-28 13:47:40\",\"opentimestamp\":\"1511848060\"}}";
