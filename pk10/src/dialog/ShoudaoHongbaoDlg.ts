class ShoudaoHongbaoDlg extends BaseDialog {
    ...
    private reloadBetLogList(dateStr: string, page: number, row: number) {
        ...
        HttpEngine.getInstance().receiveLogs(dateStr, page, row, function (name: string, response: ResReceivelogs) {
            if (response.rows.length != 0) {
                this.NameLabel.text = userInfo.nickname + "共抢到";
                this.totalLabel.text = response.total + "元";
                this.GetLebel.text ="抢到" + response.count + "个红包";
                this.totalLabel.visible = true;
                this.NameLabel.visible = true;
                this.GetLebel.visible = true;
                this.errorLebel.visible = false;
