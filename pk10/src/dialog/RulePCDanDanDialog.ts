class RulePCDanDanDialog extends BaseDialog {
    ...
    public constructor(tab_type: string, room_type: string, sound_on: number, Game_id: number, is_official: number) {
        ...
        this.skinName = "resource/skins/dialog/CommonPCDanDanRuleDialog.exml";
    ...
    public createGameScene() {
        ...
        /*
                ...
                */
        if (this.tab_type == "officalResult") {
            this.GameScroller.viewport.validateNow();
            this.GameScroller.viewport.scrollV = 0;
            //this.GameGroup.removeChildren();
            //this.rewardSelectBgImage.source = "betlistbtn_s_png";
            ...
            tween.wait(50).call(this.showOfficialResultHandler, null);
            ...
            this.menImage.source = "";
            this.gameRuleButton.skinName = "resource/skins/dialog/RuleViewPCDanDan.exml";
            this.GameGroup.addChild(this.gameRuleButton);
            let tween = egret.Tween.get(this);
    ...
    private getNewOfficalResults(name: string, response: ResPcDanDanResult): void {
        this.offcialResultItemList = new Array();
        for (var row in response.rows) {
            let item: OpenPcDanDanResults = response.rows[row] as OpenPcDanDanResults;
            var data = {
                gameInfo: item,
                expect: item.expect,
    ...
    private updateOfficalResults() {
        ...
        for (var i = 0; i < this.offcialResultItemList.length; i++) {
            ...
            BtnofficialSelec.skinName = "resource/skins/dialog/rewardPcDanDnaDlg.exml";
            BtnofficialSelec.x = 0;
            BtnofficialSelec.y = 100 * i;
            this.GameGroup.addChild(BtnofficialSelec);
            var rewardSelecGaoup: eui.Group = BtnofficialSelec.getChildByName("RewardSelecGaoup") as (eui.Group);
            var noerLabel: eui.Label = rewardSelecGaoup.getChildByName("NoerLabel") as (eui.Label)
            var TimeLabel: eui.Label = rewardSelecGaoup.getChildByName("TimeLabel") as (eui.Label);
            var code_1: eui.Image = rewardSelecGaoup.getChildByName("code_1") as (eui.Image)
            var code_2: eui.Image = rewardSelecGaoup.getChildByName("code_2") as (eui.Image)
            var code_3: eui.Image = rewardSelecGaoup.getChildByName("code_3") as (eui.Image)
            var code_4: eui.Image = rewardSelecGaoup.getChildByName("code_4") as (eui.Image)
            var code_5: eui.Image = rewardSelecGaoup.getChildByName("code_5") as (eui.Image)
            var code_6: eui.Image = rewardSelecGaoup.getChildByName("code_6") as (eui.Image)
            noerLabel.text = item.expect + "期";
            TimeLabel.text = item.opentime;
            code_1.source = String("1_1_" + item.code1 + "_png");
            code_2.source = String("1_1_" + item.code2 + "_png");
            code_3.source = String("1_1_" + item.code3 + "_png");
            code_4.source = String("1_1_" + item.he + "_png");
            if (item.dx == 1) {
                code_5.source = "ic_x_png";
            } else if (item.dx == 2) {
                code_5.source = "ic_d_png";
            }
            if (item.ds == 1) {
                code_6.source = "ic_d1_png";
            } else if (item.ds == 2) {
                code_6.source = "ic_s_png";
            }
    ...
    */
    private showOfficialResultHandler(evt: egret.TouchEvent): void {
        ...
        this.GameScroller.viewport.scrollV = 0;
        this.GameGroup.removeChildren();
        HttpEngine.getInstance().getOpenPcDanDanResult(this.Game_id, this.row, this.getNewOfficalResults, this)
        //this.rewardSelectBgImage.source = "";
        this.officialSelectBgImage.source = "betlistbtn_s_png";
        this.ruleSelectBgImage.source = "";
        this.menImage.source = "";
